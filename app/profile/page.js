...
export default function ProfilePage() {
        ...
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="flex items-center gap-3">
                    ...
                    <span style={{ color: '#4b5563' }}>Loading profile...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            {/* Cover Image - Separate from info */}
            <div className="card overflow-hidden">
                {/* Cover Image */}
                {user.cover_pic ? (
                    <div
                        className="h-50 w-auto relative"
                        style={{
                    ...
                    <div className="h-50 w-auto" style={{ background: 'linear-gradient(to right, #673DE6, #9333ea)' }}></div>
                )}
            </div>

            {/* Profile Info Card - Completely separate white card */}
            <div className="card" style={{ backgroundColor: '#ffffff', marginTop: '-60px', position: 'relative', zIndex: 10 }}>
                <div style={{ padding: '24px' }}>
                    <div className="flex flex-col md:flex-row gap-6">
                        ...
                        {/* Name & Info - All on white background */}
                        <div className="flex-1 text-center md:text-left">
                            <h1 style={{ color: '#1f2937', fontSize: '1.75rem', fontWeight: '700', lineHeight: '1.2' }}>
                                {user.owner_name || 'User'}
                            </h1>
                            <p style={{ color: '#673DE6', fontSize: '1.125rem', fontWeight: '600', marginTop: '4px' }}>
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-4">
                                {user.email && (
                                    <span className="flex items-center gap-2" style={{ color: '#4b5563', fontSize: '0.875rem' }}>
                        ...
                        <div className="flex-shrink-0 flex justify-center md:justify-end">
                            <span
                                className="rounded-full text-sm font-semibold inline-flex items-center gap-2 h-fit"
                                style={{
                                    padding: '10px 20px',
                                    backgroundColor: user.status === 1 ? '#dcfce7' : '#f3f4f6',
                                    color: user.status === 1 ? '#166534' : '#374151'
                                }}
                            >
                                <span
                                    ...
                                    style={{
                                        width: '8px',
                                        height: '8px',
                                        backgroundColor: user.status === 1 ? '#22c55e' : '#9ca3af'
                                    }}
                                ></span>
                                {user.status === 1 ? 'Active Account' : 'Inactive'}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="card p-6">
                    <h2 style={{ color: '#111827', fontSize: '1.125rem', fontWeight: '600', marginBottom: '16px' }} className="flex items-center gap-2">
                        <svg className="w-5 h-5" style={{ color: '#673DE6' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="space-y-4">
                            {invoiceSettings.shop_logo && (
                                <div className="flex items-center gap-3 mb-4">
                                    <img
                                        src={invoiceSettings.shop_logo}
                                        alt="Shop Logo"
                                        className="rounded-lg object-contain"
                                        style={{ width: '64px', height: '64px', backgroundColor: '#f9fafb', padding: '8px' }}
                                    />
                                    <div>
                                        <p style={{ color: '#111827', fontWeight: '500' }}>{invoiceSettings.shop_name}</p>
                                        <p style={{ color: '#6b7280', fontSize: '0.875rem' }}>{invoiceSettings.slogan}</p>
                                    </div>
                                </div>
                            )}
                    ...
                    <div className="flex flex-wrap gap-3">
                        {invoiceSettings.social_link && (
                            <a
                                ...
                                style={{ padding: '8px 16px', backgroundColor: '#eff6ff', color: '#1d4ed8' }}
                            >
...
// Info Row Component - using inline styles for guaranteed visibility
function InfoRow({ label, value, isLink = false, isMono = false }) {
    if (!value || value === 'null') return null;

    return (
        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
            <span style={{ color: '#6b7280', fontSize: '0.875rem', minWidth: '120px' }}>{label}</span>
            {isLink ? (
                <a
                    href={String(value).startsWith('http') ? value : `https://${value}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{ color: '#673DE6', fontSize: '0.875rem', fontWeight: '500' }}
                ...
                <span
                    style={{
                        color: '#111827',
                        fontSize: isMono ? '0.75rem' : '0.875rem',
                        fontWeight: '500',
